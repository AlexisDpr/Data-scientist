...
handle_number = handle.length
puts "Il y a #{handle.length} dans cette array."
puts ""
puts "Quel est le handle le plus court de cette liste ?"
shortest_string = handle.select{|str| str.length <5}
puts "il y a #{shortest_string.length} chaines de moins de 5 caracteres qui est #{shortest_string.sort}"
puts ""
puts "Combien y-a-t'il de handle contenant 5 caractères"
longest_string = handle.select{|str| str.length >= 5}
puts "il y a #{longest_string.length} handle de plus de 5 caracteres"
puts ""
puts "Combien commencent par une majuscule"
count = handle.select{|str| str =~ /@([A-Z])/}.count
puts "Il y a #{count} handle qui commencent par une majuscule après le @."
puts ""
puts "Trie la liste de handle par taille des handle (petit au plus grand)"
sorted_string = handle.sort_by{|str| str.split("@").last.downcase}
puts sorted_string
puts ""
puts "Quelle est la position dans l'array de la personne @epenser ?"
index_in_array = handle.index("@epenser")
puts "@epenser se trouve à la #{index_in_array} position."
puts ""
